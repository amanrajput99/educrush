    ...
    >
      <div className="flex items-center justify-between px-4 py-3 bg-[#0d0e0e] border-b border-slate-800/70">
        <div className="flex items-center gap-3">
          ...
          <div className="px-2 py-0.5 rounded text-[10px] font-black" style={{ background: s.bg, color: s.dot }}>PDF</div>
          <span className="text-slate-300 text-[13px] font-medium truncate max-w-[200px] sm:max-w-sm">{note.title}</span>
        </div>
      ...
      <div className="relative" style={{ paddingBottom: '72%', minHeight: '480px' }}>
        <AnimatePresence>
          {!loaded && (
            <motion.div key="loader" exit={{ opacity: 0 }} transition={{ duration: 0.4 }} className="absolute inset-0 flex flex-col items-center justify-center bg-[#08090a] z-10">
              <div className="relative mb-6">
                <div className="flex gap-0.5">
                  <div className="w-3 rounded-l-sm" style={{ background: s.dot + '80', height: '80px' }} />
                  <div className="w-16 rounded-r-xl border-2 flex flex-col justify-center gap-2 px-2.5 py-3" style={{ borderColor: s.dot + '40', background: s.bg, height: '80px' }}>
                    {[80, 60, 75, 50].map((w, i) => (
                      <motion.div key={i} className="h-1.5 rounded-full" style={{ background: s.dot + '50', width: `${w}%` }} animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 1.5, delay: i * 0.2, repeat: Infinity }} />
                    ))}
                  </div>
                </div>
                <motion.div className="absolute -inset-6 rounded-full blur-2xl" style={{ background: s.glow }} animate={{ opacity: [0.08, 0.18, 0.08] }} transition={{ duration: 2, repeat: Infinity }} />
              </div>
        ...
        <iframe src={embedUrl} className="absolute inset-0 w-full h-full" style={{ border: 'none' }} allow="autoplay" onLoad={() => setLoaded(true)} title={note.title} />
      </div>
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-[#0a0b0b] border-t border-slate-800/50">
        ...
        <div className="flex items-center gap-1.5">
          <motion.span className="w-1.5 h-1.5 rounded-full" style={{ background: loaded ? '#4ade80' : '#475569' }} animate={loaded ? { scale: [1, 1.3, 1] } : {}} transition={{ duration: 1, repeat: loaded ? 0 : Infinity }} />
          <span className="text-[11px]" style={{ color: loaded ? '#4ade80' : '#475569' }}>{loaded ? 'Ready' : 'Loading...'}</span>
        </div>
      </div>
    </motion.div>
  )
...
const RelatedCard = ({ note, index }: { note: Note; index: number }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.06 }}>
      <Link href={note.link} className="group flex items-start gap-3 p-3.5 rounded-xl border border-slate-800 bg-[#0c0c0e] hover:border-slate-700 hover:bg-[#111213] transition-all duration-200">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 text-[10px] font-black" style={{ background: s.bg, color: s.dot }}>
          {note.subject.slice(0, 2).toUpperCase()}
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-white text-[13px] font-medium leading-snug line-clamp-2 group-hover:text-[#c8fa45] transition-colors">{note.title}</p>
    ...
    return (
      <main className="min-h-screen bg-black pt-24 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <Link href="/notes" className="px-5 py-2.5 rounded-xl text-sm font-semibold transition-all" style={{ background: '#c8fa4518', border: '1px solid #c8fa4535', color: '#c8fa45' }}>
            ← Back to Notes
          </Link>
        </div>
      </main>
    ...
    <>
      ...
      <main className="min-h-screen bg-black text-white pt-20 pb-24">
        <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
          <div className="absolute -top-40 left-1/2 -translate-x-1/2 w-[800px] h-[500px] rounded-full blur-[140px]" style={{ background: s.glow, opacity: 0.05 }} />
        </div>
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
            ...
            <div className="min-w-0">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }} className="mb-7">
                <div className="flex flex-wrap items-center gap-2 mb-4">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold" style={{ background: s.bg, color: s.dot, border: `1px solid ${s.dot}30` }}>
                    <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: s.dot }} />{note.subject}
                  </span>
                  {note.course && <span className="px-3 py-1 rounded-full border border-slate-700/60 bg-slate-900/60 text-slate-400 text-[11px]">{note.course}</span>}
                  {note.year && <span className="px-3 py-1 rounded-full border border-slate-700/60 bg-slate-900/60 text-slate-400 text-[11px]">{note.year}</span>}
                ...
                <div className="flex flex-wrap gap-2.5 mb-5">
                  <StatPill icon={<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>} label="Format" value="PDF Notes" />
                  <StatPill icon={<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></svg>} label="Course" value={note.course ?? 'General'} />
                  {note.year && <StatPill icon={<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>} label="Year" value={note.year} />}
                </div>

                <div className="flex flex-wrap gap-2">
                  <a href={note.drivelink ?? note.link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all" style={{ background: '#c8fa45', color: '#0a0a0a' }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                    Download PDF
                  </a>
                  <button onClick={handleShare} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold border border-slate-700 text-slate-300 hover:border-slate-500 hover:text-white transition-all">
                    {copyDone ? (
                      <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#4ade80" strokeWidth="2.5"><path d="M20 6 9 17l-5-5"/></svg><span className="text-green-400">Copied!</span></>
                    ) : (
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mt-8 rounded-2xl border border-slate-800 bg-[#0c0c0e] overflow-hidden">
                ...
                <div className="p-5">
                  <p className="text-slate-400 text-[14px] leading-relaxed mb-4">{note.description}</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {['Clear concept explanations', 'Exam-focused important topics', 'Easy to understand language', 'Diagrams & examples included', 'Previous year question hints', 'Concise revision-ready format'].map((item, i) => (
                      <div key={i} className="flex items-center gap-2.5 text-[13px] text-slate-400">
                        <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: s.dot }} />{item}
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {/* RIGHT SIDEBAR */}
            <motion.aside initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.45, delay: 0.2 }} className="space-y-5 lg:sticky lg:top-24 lg:self-start">
              <div className="rounded-2xl border border-slate-800 bg-[#0c0c0e] overflow-hidden" style={{ boxShadow: `0 0 40px ${s.glow}08` }}>
                <div className="px-4 py-3 border-b border-slate-800/60 flex items-center gap-2" style={{ background: s.bg + '50' }}>
                ...
                <div className="px-4 pb-4 space-y-2">
                  <a href={note.drivelink ?? note.link} target="_blank" rel="noreferrer" className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold transition-all" style={{ background: '#c8fa45', color: '#0a0a0a' }}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                    Download PDF
                  </a>
                  <a href={note.drivelink ?? note.link} target="_blank" rel="noreferrer" className="w-full flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-semibold border border-slate-700 text-slate-400 hover:border-slate-500 hover:text-white transition-all">
                    Open in Drive
                  </a>
                </div>
              </div>
