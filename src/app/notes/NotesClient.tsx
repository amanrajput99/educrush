      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        {(note.year || note.course) && (
          <div className="flex gap-1.5 mb-2 flex-wrap">
            {note.course && <span className="text-[10px] px-2 py-0.5 rounded-full border border-slate-700 text-slate-500">{note.course}</span>}
            {note.year && <span className="text-[10px] px-2 py-0.5 rounded-full border border-slate-700 text-slate-500">{note.year}</span>}
          </div>
        )}

        <h3 className="text-[15px] font-semibold text-white mb-1 leading-snug">{note.title}</h3>
        <p className="text-[12px] text-slate-500 leading-relaxed line-clamp-2 mb-3">{note.description}</p>
...
export default function NotesClient({ initialNotes }: { initialNotes: Note[] }) {
  ...
  const handleYearChange = (year: string) => { setActiveYear(year); setActiveSubject('All') }

  const yearOptions = useMemo(() => activeCourse === 'All' ? [] : COURSE_YEARS[activeCourse] ?? [], [activeCourse])
  ...
  const filtered = useMemo(() => noteList.filter((n) => {
    ...
    const matchSearch  = !q || n.title.toLowerCase().includes(q) || n.description.toLowerCase().includes(q) || n.subject.toLowerCase().includes(q)
    return matchCourse && matchYear && matchSubject && matchSearch
  }), [noteList, activeCourse, activeYear, activeSubject, search])

  const clearAll = () => { setSearch(''); setActiveCourse('All'); setActiveYear('All'); setActiveSubject('All') }
    ...
    <main className="min-h-screen bg-black pt-24 pb-20 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-14">
          <button className="px-4 h-8 border border-gray-800 text-slate-200 text-xs rounded-lg mb-4">Notes</button>
          <h1 className="text-5xl md:text-[56px] font-medium text-white tracking-tighter mt-2">All Notes</h1>
        ...
        <div className="mb-1">
          <p className="text-[11px] text-slate-600 uppercase tracking-widest mb-2.5 font-semibold">Select Course</p>
          <div className="flex flex-wrap gap-2">
            <FilterPill label="All" active={activeCourse === 'All'} onClick={() => handleCourseChange('All')} />
            {COURSES.map((course) => <FilterPill key={course} label={course} active={activeCourse === course} onClick={() => handleCourseChange(course)} />)}
          </div>
        </div>

        <AnimatePresence>
          {activeCourse !== 'All' && yearOptions.length > 0 && (
            <motion.div initial={{ opacity: 0, height: 0, marginTop: 0 }} animate={{ opacity: 1, height: 'auto', marginTop: 20 }} exit={{ opacity: 0, height: 0, marginTop: 0 }} transition={{ duration: 0.22 }} className="overflow-hidden">
              <p className="text-[11px] text-slate-600 uppercase tracking-widest mb-2.5 font-semibold">Select Year</p>
              <div className="flex flex-wrap gap-2">
                <FilterPill label="All Years" active={activeYear === 'All'} onClick={() => handleYearChange('All')} />
                {yearOptions.map((year) => <FilterPill key={year} label={year} active={activeYear === year} onClick={() => handleYearChange(year)} />)}
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {subjectOptions.length > 2 && (
            <motion.div initial={{ opacity: 0, height: 0, marginTop: 0 }} animate={{ opacity: 1, height: 'auto', marginTop: 20 }} exit={{ opacity: 0, height: 0, marginTop: 0 }} transition={{ duration: 0.22 }} className="overflow-hidden">
              <p className="text-[11px] text-slate-600 uppercase tracking-widest mb-2.5 font-semibold">Select Subject</p>
              <div className="flex flex-wrap gap-2">
                {subjectOptions.map((subject) => <FilterPill key={subject} label={subject} active={activeSubject === subject} onClick={() => setActiveSubject(subject)} />)}
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="text-sm text-slate-600 flex items-center gap-2 flex-wrap">
            <span>{filtered.length === 0 ? 'No notes found' : `${filtered.length} note${filtered.length !== 1 ? 's' : ''}`}</span>
            {activeCourse !== 'All' && <span className="px-2 py-0.5 rounded-full bg-slate-900 border border-slate-800 text-[11px] text-slate-500">{activeCourse}</span>}
            {activeYear !== 'All' && <span className="px-2 py-0.5 rounded-full bg-slate-900 border border-slate-800 text-[11px] text-slate-500">{activeYear}</span>}
            {activeSubject !== 'All' && <span className="px-2 py-0.5 rounded-full bg-slate-900 border border-slate-800 text-[11px] text-slate-500">{activeSubject}</span>}
          </p>
          {hasActiveFilters && (
            <button onClick={clearAll} className="text-xs text-[#c8fa45] hover:text-[#d6ff55] flex items-center gap-1 transition-colors shrink-0">
