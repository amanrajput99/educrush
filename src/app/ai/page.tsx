...
const PROVIDERS = [
  { id: "groq"      as Provider, name: "Groq",   model: "llama-3.1-8b-instant", badge: "Free", color: "#F55036" },
  { id: "gemini"    as Provider, name: "Gemini", model: "gemini-1.5-flash",      badge: "Free", color: "#4285F4" },
...
export default function AiChatPage() {
  ...
  // ── useChat — @ai-sdk/react v3 + ai v6 correct API ────────────────────────
  ...
  const scrollToBottomIfNeeded = useCallback(() => {
    if (!isNearBottomRef.current || ticking.current) return;
  ...
  useEffect(() => { scrollToBottomIfNeeded(); }, [messages, scrollToBottomIfNeeded]);
  ...
  // UIMessage v6: content lives in `parts` array
  ...
  async function handleSend(text?: string) {
    ...
    setRows(1);
    forceScrollToBottom();
    // ✅ sendMessage correct signature for @ai-sdk/react v3
  ...
  return (
    <div
      ...
      style={{ minHeight: "calc(100vh - 64px)", background: "#060a06" }}
      onClick={() => setShowProviders(false)}
    >
      {/* Greenish ambient glow */}
      <div aria-hidden className="pointer-events-none absolute inset-0 z-0" style={{
        background:
          "radial-gradient(ellipse 70% 55% at 50% 0%, rgba(34,197,94,0.09) 0%, transparent 70%)," +
          "radial-gradient(ellipse 50% 40% at 80% 80%, rgba(16,185,129,0.06) 0%, transparent 60%)," +
          "radial-gradient(ellipse 40% 30% at 10% 60%, rgba(52,211,153,0.05) 0%, transparent 60%)",
      }} />
      {/* Scanline */}
      <div aria-hidden className="pointer-events-none absolute inset-0 z-0 opacity-[0.025]" style={{
        backgroundImage: "repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(74,222,128,0.3) 2px, rgba(74,222,128,0.3) 3px)",
        backgroundSize: "100% 4px",
      }} />

      {/* Chat area */}
      <div ref={scrollContainerRef} onScroll={checkNearBottom} className="relative z-10 flex-1 overflow-y-auto px-4 py-6">
        <div className="max-w-2xl mx-auto flex flex-col gap-6">
          ...
          {/* Model switcher (top-right when chatting) */}
          {messages.length > 0 && (
          ...
          {messages.map((m, i) => (
            <div key={i} className={`flex gap-3 ${m.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
              <div
                className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-semibold mt-0.5 ${m.role === "user" ? "bg-violet-600 text-white" : ""}`}
                style={m.role === "assistant" ? {
                  background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.25)",
                  color: "rgba(134,239,172,1)", boxShadow: "0 0 12px rgba(34,197,94,0.15)",
                } : {}}
              >
                {m.role === "user" ? "U" : "✦"}
              </div>
              <div
                className={`max-w-[78%] px-4 py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${
                  m.role === "user" ? "bg-violet-600 text-white rounded-tr-sm" : "rounded-tl-sm text-white/85"
                }`}
                style={m.role === "assistant" ? {
          ...
          {isLoading && (
            <div className="flex gap-3">
              ...
              <div className="px-4 py-3 rounded-2xl rounded-tl-sm flex items-center gap-1.5" style={{
                background: "rgba(255,255,255,0.04)", border: "1px solid rgba(34,197,94,0.12)",
              }}>
                {[0, 150, 300].map((d) => (
      ...
      <div className="relative z-10 px-4 pb-6 pt-2 border-t border-white/[0.06]">
        <div aria-hidden className="pointer-events-none absolute bottom-0 left-1/2 -translate-x-1/2 w-[60%] h-24 -z-10"
          style={{ background: "radial-gradient(ellipse at center bottom, rgba(34,197,94,0.08) 0%, transparent 70%)" }} />
        <div className="max-w-2xl mx-auto">
          <div className="flex items-end gap-2 rounded-2xl px-4 py-3" style={{
            background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)",
          }}>
            <textarea
              rows={rows} value={input}
