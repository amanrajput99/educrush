          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {codeBlocks.map((block, i) => {
                ...
                return (
                  <button key={i} onClick={() => setActiveTab(i)} className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold border transition-all duration-200"
                    style={isActive ? { background: s.bg, color: s.text, borderColor: s.dot + '60', boxShadow: `0 0 12px ${s.dot}20` } : { background: 'transparent', color: '#64748b', borderColor: '#1e293b' }}>
                    <span className="w-2 h-2 rounded-full" style={{ background: isActive ? s.dot : '#334155' }} />
                    {block.language}
                    <span className="font-mono text-[10px] px-1.5 py-0.5 rounded" style={{ background: isActive ? s.dot + '25' : '#0f172a', color: isActive ? s.text : '#475569' }}>
                      {block.filename}
                    </span>
                  </button>
                )
