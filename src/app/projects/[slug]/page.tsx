  ...
  <pre className="p-5 text-[13px] leading-[1.8] font-mono text-slate-300 min-w-full">
          <code
            dangerouslySetInnerHTML={{
              __html: highlight(block.code, block.language),
            }}
          />
        </pre>
      </div>
...
// for zip file download
  ...
  project.codeBlocks?.forEach((block) => {
    folder.file(block.filename, block.code)
  })
...
<motion.div
  ...
  className="rounded-2xl overflow-hidden border border-slate-800 mb-8 relative group"
>
  <img
    src={project.image}
    alt={project.name}
    ...
    onError={(e) => {
      e.currentTarget.src = 'https://images.unsplash.com/photo-1629752187687-3d3c7ea3a21b?w=1200'
    }}
  />
  {/* Gradient overlay */}
  ...
  <div className="absolute bottom-5 right-5 flex gap-2">
    ...
    {project.codeBlocks && project.codeBlocks.length > 0 && (
      <button
        ...
        className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-500/50 bg-slate-900/70 text-slate-200 text-sm font-semibold backdrop-blur-sm hover:bg-slate-800 hover:border-slate-400 transition-all"
      >
    ...
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-emerald-400/50 bg-emerald-500/20 text-emerald-200 text-sm font-semibold backdrop-blur-sm hover:bg-emerald-500/30 transition-all"
   >
      Live Preview
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
        ...
        >
          ...
          {/* Title */}
          <h1 className="text-3xl sm:text-4xl font-semibold text-white tracking-tighter leading-tight mb-3">
            {project.name}
          </h1>
                ...
                return (
                  ...
                  >
                    <span
                      ...
                      style={{ background: isActive ? s.dot : '#334155' }}
                    />
                    {block.language}
                    <span
                      className="font-mono text-[10px] px-1.5 py-0.5 rounded"
                      style={{
                        background: isActive ? s.dot + '25' : '#0f172a',
                        color: isActive ? s.text : '#475569',
                      }}
                    >
                      {block.filename}
                    </span>
        ...
        <motion.div
          ...
          className="mt-14 pt-6 border-t border-slate-800/50 flex items-center justify-between"
        >
          <Link
            href="/projects"
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-sm font-medium hover:bg-emerald-500/20 transition-all"
          >
            Open Live Project
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
