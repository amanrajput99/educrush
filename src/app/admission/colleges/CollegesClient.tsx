...
function CollegeCard({ college, index, compareList, onCompareToggle }: {
  ...
  return (
    <motion.div
      ...
      className={`group relative bg-slate-950 border rounded-2xl p-6 transition-all duration-300 flex flex-col
        ${isComparing ? 'border-emerald-500/60 shadow-[0_0_20px_rgba(52,211,153,0.1)]' : 'border-slate-800 hover:border-emerald-700/40'}`}
    >
      {/* Hover glow */}
      <AnimatePresence>
        {hovered && (
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-900/60 to-slate-900 border border-emerald-900/40 flex items-center justify-center text-2xl">
          🏛️
        </div>
        {college.badge && (
          <span className={`text-[10px] px-2.5 py-0.5 rounded-full border ${BADGE_STYLES[college.badge] ?? 'bg-slate-900 border-slate-700 text-slate-400'}`}>
            {college.badge}
          </span>
        )}
      </div>

      <h3 className="font-bold text-white text-base mb-1 group-hover:text-emerald-300 transition-colors">{college.name}</h3>
      ...
      <div className="grid grid-cols-2 gap-3 mb-5 text-xs">
        <div className="bg-black/40 rounded-xl p-3">
          ...
          <div className="text-emerald-400 font-semibold">₹{(college.fees_min / 1000).toFixed(0)}k–{(college.fees_max / 1000).toFixed(0)}k</div>
        </div>
        {college.nirf_rank ? (
          <div className="bg-black/40 rounded-xl p-3">
            <div className="text-slate-600 mb-0.5">NIRF Rank</div>
            <div className="text-white font-semibold">#{college.nirf_rank}</div>
          </div>
        ) : college.placement_avg ? (
          <div className="bg-black/40 rounded-xl p-3">
            <div className="text-slate-600 mb-0.5">Avg Placement</div>
            <div className="text-white font-semibold">₹{college.placement_avg}L</div>
          </div>
        ) : null}
      </div>
      ...
      <div className="mt-auto flex gap-2">
        ...
        <Link
          href={`/admission/colleges/${college.slug}`}
          className="flex-1 py-2.5 rounded-xl text-xs font-semibold bg-emerald-500 hover:bg-emerald-400 text-black text-center transition-all duration-200 hover:scale-[1.02]"
        >
          Details →
        </Link>
      </div>
...
export default function CollegesClient({ colleges }: { colleges: College[] }) {
  ...
  const allCourses = useMemo(() => {
    ...
    colleges.forEach(c => c.courses.forEach(co => s.add(co)))
    return ['All', ...Array.from(s)]
  }, [colleges])

  const allCities = useMemo(() => {
    const s = new Set<string>()
    colleges.forEach(c => s.add(c.city))
    return ['All', ...Array.from(s)]
  }, [colleges])
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="relative pt-28 pb-12 px-4 text-center overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} className="inline-flex mb-5">
          <span className="px-4 h-8 flex items-center border border-emerald-900/60 bg-emerald-950/30 text-emerald-400 text-xs rounded-full">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="relative max-w-lg mx-auto">
          ...
          <input
            type="text"
            placeholder="College naam ya city search karo..."
            value={search}
            onChange={e => setSearch(e.target.value)}
            className="w-full pl-11 pr-4 py-3.5 rounded-2xl bg-slate-900 border border-slate-800 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-emerald-500/50 transition-all"
          />
        </motion.div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 pb-32">
        {filtered.length === 0 ? (
          <div className="text-center py-24">
            <p className="text-slate-500">Koi college nahi mila — search ya filter change karo.</p>
          </div>
        ) : (
          ...
          >
            <div className="max-w-2xl mx-auto bg-slate-950/95 border border-emerald-800/60 backdrop-blur-xl rounded-2xl p-4 flex items-center justify-between shadow-[0_0_40px_rgba(52,211,153,0.15)]">
              <div className="text-sm text-white">
                <span className="text-emerald-400 font-semibold">{compareList.length} colleges</span> selected for comparison
              </div>
              <div className="flex gap-2">
                ...
                <button className="text-xs px-5 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-black font-bold transition-all hover:scale-105">
                  Compare Now →
                </button>
              </div>
            </div>
